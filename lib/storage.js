...
function cleanLegacyDemoBranding(payload) {
  const next = {
    clinics: (payload.clinics || []).map((clinic) => ({
      ...clinic,
      name: clinic.id === "demo-clinic" && clinic.name === "Praarambh IVF Demo"
        ? "Praarambh IVF Main"
        : clinic.name
    })),
    users: (payload.users || []).map((user) => ({
      ...user,
      clinicName:
        user.clinicId === "demo-clinic" && user.clinicName === "Praarambh IVF Demo"
          ? "Praarambh IVF Main"
          : user.clinicName,
      name: user.username === "admin" && user.name === "Demo Admin" ? "Clinic Admin" : user.name
    })),
    patients: (payload.patients || []).map((patient) => ({
      ...patient,
      patientName: patient.patientName === "Live Demo Patient" ? "Primary Patient" : patient.patientName
    })),
    logs: (payload.logs || []).map((log) => ({
      ...log,
      user: log.user === "Demo Admin" ? "Clinic Admin" : log.user,
      message: String(log.message || "")
        .replaceAll("Praarambh IVF Demo", "Praarambh IVF Main")
        .replaceAll("Live Demo Patient", "Primary Patient")
        .replaceAll("Demo Admin", "Clinic Admin")
    }))
  };
...
class JsonStorage {
  ...
  async init() {
    ...
    if (!clinics.length) {
      ...
      const cleaned = cleanLegacyDemoBranding({
        clinics,
        users: readJson(this.files.users, []),
        patients: readJson(this.files.patients, []),
        logs: readJson(this.files.logs, [])
      });
      clinics = cleaned.clinics.map(normalizeClinicRecord);
      writeJson(this.files.users, cleaned.users);
      writeJson(this.files.patients, cleaned.patients);
      writeJson(this.files.logs, cleaned.logs);
      writeJson(this.files.clinics, clinics);
    }
...
class PostgresStorage {
  ...
  async init() {
    ...
    await this.pool.query(`
      UPDATE clinics
      SET name = 'Praarambh IVF Main'
      WHERE id = 'demo-clinic' AND name = 'Praarambh IVF Demo';

      UPDATE users
      SET clinic_name = 'Praarambh IVF Main'
      WHERE clinic_id = 'demo-clinic' AND clinic_name = 'Praarambh IVF Demo';

      UPDATE users
      SET name = 'Clinic Admin'
      WHERE username = 'admin' AND name = 'Demo Admin';

      UPDATE patients
      SET patient_name = 'Primary Patient'
      WHERE patient_name = 'Live Demo Patient';

      UPDATE activity_logs
      SET user_name = CASE WHEN user_name = 'Demo Admin' THEN 'Clinic Admin' ELSE user_name END,
          message = REPLACE(
            REPLACE(
              REPLACE(message, 'Praarambh IVF Demo', 'Praarambh IVF Main'),
              'Live Demo Patient',
              'Primary Patient'
            ),
            'Demo Admin',
    ...
    await this.pool.query(
      `
        INSERT INTO patients (
          ...
          mobile, alternate_mobile, enquiry_source, doctor_name, address, state, city, marital_status,
    ...
    await this.pool.query(
      `
        ...
        SET clinic_id = $2, uhid = $3, patient_name = $4, age = $5, dob = $6, partner_name = $7,
            ...
            enquiry_source = $12, doctor_name = $13, address = $14, state = $15, city = $16, marital_status = $17,
            years_married = $18, marriage_date = $19, previous_children = $20, child_count = $21,
            child_details = $22, consultation_for = $23::jsonb, consultation_other = $24,
            past_history = $25, reference = $26, referred_by = $27, hospital_clinic = $28,
            notes = $29, next_follow_up_date = $30, status = $31, created_at = $32, follow_ups = $33::jsonb
        WHERE id = $1
      `,
      [
        patient.id,
